	...
	public class FoodFormServlet extends HttpServlet {
		...
		public void doGet(HttpServletRequest request, HttpServletResponse response)
			...
			((HttpServletResponse)response).addHeader("Access-Control-Allow-Origin", "*");
				...
				Food_jdjc food_jdjc = CommonUtils.toBean(map, Food_jdjc.class);//把Map中大部分数据封装到Food_jdjc对象中
				/*
				 * 4.2 把上传的图片保存起来
				 *   > 获取文件名：截取之
				...
				if(fileItem.getSize() == 0){
		  		...
		  		food_jdjc.setImage_resource(image_resource);
		  		foodService.add_jdjc(food_jdjc);
		       	request.setAttribute("success", "提交成功");
		  		request.getRequestDispatcher("/FoodServlet?method=dispNameData").forward(request, response);
		}
