...
import cn.tslanpu.test.utils.TokenProccessor;
	...
	public class UsemedFormServlet extends HttpServlet {
		...
		public void doGet(HttpServletRequest request, HttpServletResponse response)
			...
			((HttpServletResponse)response).addHeader("Access-Control-Allow-Origin", "*");
				...
				Usemed_jdjc usemed_jdjc = CommonUtils.toBean(map, Usemed_jdjc.class);//把Map中大部分数据封装到Food_jdjc对象中
				/*
				 * 4.2 把上传的图片保存起来
				 *   > 获取文件名：截取之
				...
				if(fileItem.getSize() == 0){
		  		...
		  		usemed_jdjc.setImage_resource(image_resource);
		  		usemedService.add_jdjc(usemed_jdjc);
		       	request.setAttribute("success", "提交成功");
		  		request.getRequestDispatcher("/UsemedServlet?method=dispNameData").forward(request, response);
		}
