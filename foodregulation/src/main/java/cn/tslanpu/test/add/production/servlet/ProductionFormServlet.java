...
public class ProductionFormServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		((HttpServletResponse)response).addHeader("Access-Control-Allow-Origin", "*");
			...
			Production_jdjc production_jdjc = CommonUtils.toBean(map, Production_jdjc.class);//把Map中大部分数据封装到Production_jdjc对象中
			/*
			 * 4.2 把上传的图片保存起来
			 *   > 获取文件名：截取之
			...
			if(fileItem.getSize() == 0){
	  		...
	  		production_jdjc.setImage_resource(image_resource);
