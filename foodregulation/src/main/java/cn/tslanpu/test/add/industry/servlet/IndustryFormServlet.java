...
public class IndustryFormServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		((HttpServletResponse)response).addHeader("Access-Control-Allow-Origin", "*");
			...
			for(FileItem fileItem : fileItemList) {
			 ...
			 Industry_jdjc industry_jdjc = CommonUtils.toBean(map,  Industry_jdjc.class);//把Map中大部分数据封装到Production_jdjc对象中
			/*
			 * 4.2 把上传的图片保存起来
			 *   > 获取文件名：截取之
			...
			if(fileItem.getSize() == 0){
	  		...
	  		industry_jdjc.setImage_resource(image_resource);
	  		industryService.add_jdjc(industry_jdjc);
	       	request.setAttribute("success", "提交成功");
	  		request.getRequestDispatcher("/IndustryServlet?method=dispNameData").forward(request, response);
	}
