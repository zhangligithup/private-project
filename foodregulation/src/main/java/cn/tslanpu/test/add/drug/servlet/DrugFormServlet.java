...
import cn.tslanpu.test.utils.TokenProccessor;
	...
	public class DrugFormServlet extends HttpServlet {
		...
		public void doGet(HttpServletRequest request, HttpServletResponse response)
			...
			((HttpServletResponse)response).addHeader("Access-Control-Allow-Origin", "*");
				...
				Drug_jdjc drug_jdjc = CommonUtils.toBean(map, Drug_jdjc.class);//把Map中大部分数据封装到Food_jdjc对象中
				/*
				 * 4.2 把上传的图片保存起来
				 *   > 获取文件名：截取之
				...
				if(fileItem.getSize() == 0){
		  		...
		  		drug_jdjc.setImage_resource(image_resource);
		  		drugService.add_jdjc(drug_jdjc);
		       	request.setAttribute("success", "提交成功");
		  		request.getRequestDispatcher("/DrugServlet?method=dispNameData").forward(request, response);
		}
