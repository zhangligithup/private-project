...
import cn.tslanpu.test.utils.TokenProccessor;
	...
	public class MedicalFormServlet extends HttpServlet {
		...
		public void doGet(HttpServletRequest request, HttpServletResponse response)
			...
			((HttpServletResponse)response).addHeader("Access-Control-Allow-Origin", "*");
				...
				Medical_jdjc medical_jdjc = CommonUtils.toBean(map, Medical_jdjc.class);//把Map中大部分数据封装到Food_jdjc对象中
				/*
				 * 4.2 把上传的图片保存起来
				 *   > 获取文件名：截取之
				...
				if(fileItem.getSize() == 0){
		  		...
		  		medical_jdjc.setImage_resource(image_resource);
		  		medicalService.add_jdjc(medical_jdjc);
		       	request.setAttribute("success", "提交成功");
		  		request.getRequestDispatcher("/MedicalServlet?method=dispNameData").forward(request, response);
		}
