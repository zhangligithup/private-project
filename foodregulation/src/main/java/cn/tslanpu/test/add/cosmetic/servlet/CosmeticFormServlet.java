...
public class CosmeticFormServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		((HttpServletResponse)response).addHeader("Access-Control-Allow-Origin", "*");
			...
			Cosmetic_jdjc cosmetic_jdjc = CommonUtils.toBean(map, Cosmetic_jdjc.class);//把Map中大部分数据封装到Production_jdjc对象中
			/*
			 * 4.2 把上传的图片保存起来
			 *   > 获取文件名：截取之
			...
			if(fileItem.getSize() == 0){
	  		...
	  		cosmetic_jdjc.setImage_resource(image_resource);
	  		cosmeticService.add_jdjc(cosmetic_jdjc);
	       	request.setAttribute("success", "提交成功");
	  		request.getRequestDispatcher("/CosmeticServlet?method=dispNameData").forward(request, response);
	}
