	...
	public JSONObject addExamination(HttpServletRequest request, Examination examination) {
		...
		try {
			if ((StringUtils.isNumeric(examination.getChoiceScore()))
					...
					&& (StringUtils.isNumeric(examination.getCompletionScore()))
					&& (StringUtils.isNumeric(examination.getChoiceTime()))
					&& (StringUtils.isNumeric(examination.getJudgmentTime()))
					&& (StringUtils.isNumeric(examination.getCompletionTime()))) {
				if (StringUtils.isNotBlank(examination.getChoiceCount())) {
					...
					Double xuanzef = Double.valueOf(Double.parseDouble(examination.getChoiceScore()));
					Double xuanzet = Double.valueOf(Double.parseDouble(examination.getChoiceTime()));
					if ((xuanzef.doubleValue() <= 0.0D) || (xuanzet.doubleValue() <= 0.0D)) {
						return ResultUtil.jsonObject("false", "选择题分数和时间不能小于等于0");
					}
				}

				if (StringUtils.isNotBlank(examination.getJudgmentCount())) {
					...
					Double panduanf = Double.valueOf(Double.parseDouble(examination.getJudgmentScore()));
					Double panduant = Double.valueOf(Double.parseDouble(examination.getJudgmentTime()));
					if ((panduanf.doubleValue() <= 0.0D) || (panduant.doubleValue() <= 0.0D)) {
						return ResultUtil.jsonObject("false", "判断题分数和时间不能小于等于0");
					}
				}

				if (StringUtils.isNotBlank(examination.getCompletionCount())) {
					...
					Double tiankongf = Double.valueOf(Double.parseDouble(examination.getCompletionScore()));
					Double tiankongt = Double.valueOf(Double.parseDouble(examination.getCompletionTime()));
					if ((tiankongf.doubleValue() <= 0.0D) || (tiankongt.doubleValue() <= 0.0D)) {
						return ResultUtil.jsonObject("false", "填空题分数和时间不能小于等于0");
					}
				}

				int add = this.examinationService.addExamination(examination);

				if (add > 0) {
					List<ExamRelation> list = new ArrayList<ExamRelation>();
					if (examination.getChoiceList() != null) {
					...
					if (examination.getJudgmentList() != null) {
					...
					if (examination.getCompletionList() != null) {
					...
					this.examRelationService.insertList(list);
