...
<div id="div_img" class="row-fluid">

</div>
<div id="zyupload" class="zyupload row-fluid" style="min-height:400px">
    
</div>
<input type="hidden" name="imgUrl" value="${supervisionAndCheck.imgUrl}">
<input type="hidden" id="inspectionRecordId" name="inspectionRecordId" value="${supervisionAndCheck.inspectionRecordId}">
<input type="hidden" id="proposalId" name="proposalId" value="${supervisionAndCheck.proposalId}">
<input type="hidden" id="noticeId" name="noticeId" value="${supervisionAndCheck.noticeId}">
</form>
<table class="table search_box marb10">
	<thead>
		<tr>
		     <th>
		     	<input id="inspectionRecord" name="" class="input2" size="16" type="text" readonly value="${supervisionAndCheck.inspectionRecordStr}">
		     </th>
		     ...
		     <th><a href="javascript:void(0)" class="form_btn" id="inspectionRecordPdf" download>下载pdf文档</a></th>
		     <th><a href="javascript:void(0)" class="form_btn" onclick="deleteInspectionRecord()">删除</a></th>
		</tr>
		<tr>
		     <th>
		     	<input id="proposal" name="" class="input2" size="16" type="text" readonly value="${supervisionAndCheck.proposalStr}">
		     </th>
		     ...
		     <th><a href="javascript:void(0)" class="form_btn" id="proposalPdf" download>下载pdf文档</a></th>
		     <th><a href="javascript:void(0)" class="form_btn" onclick="deleteProposal()">删除</a></th>
		</tr>
		<tr>
		     <th>
		     	<input id="notice" name="" class="input2" size="16" type="text"  readonly value="${supervisionAndCheck.noticeStr}">
		     </th>
		     ...
		     <th><a href="javascript:void(0)" class="form_btn" id="noticePdf" download>下载pdf文档</a></th>
        ...
        <div class="modal-content">
            ...
            <table class="table search_box marb10">
                <thead>
                    <tr>
                        <th>违法条款编号</th>
                        <th><input id="searchIllegalClauseCode2" class="search_input"
                            type="text" value=""></th>
                        <th><a class="form_btn" href="#"
                            onclick="queryChooseIllegalClause2()">搜索</a></th>
                    </tr>
                </thead>
            </table>
            <div class="modal-body">
				<table class="table table-bordered table2">
			        ...
			        <tbody id="dataListTbody_chooseIllegalClause2">
			
			        </tbody>
			    </table>
			     <div id="page_illegalClause2"></div>
            </div>
            <div class="modal-footer">
...
$(document).ready(function(){
	queryChooseIllegalClause2();
	loadImg();
	//加载pdf路径
	if($("#inspectionRecord").val()){
		inspectionRecordPdf(0);
	}
	if($("#proposal").val()){
		proposalPdf(0);
	}
...
function inspectionRecordPdf(type){
	$.ajax({
        url:"${pageContext.request.contextPath}/supervisionAndCheck/getInspectionRecordPdfUrl.do",
        type:"post",
        data:{
        	id:$("#inspectionRecordId").val(),
        	type:type
        },
        success : function(url) {
            if(url){
            	$("#inspectionRecordPdf").attr("href","http://lanpubucket1.oss-cn-beijing.aliyuncs.com/"+url);
            }
        }
    }); 
}
function proposalPdf(type){
	   $.ajax({
	        url:"${pageContext.request.contextPath}/supervisionAndCheck/getProposalPdfUrl.do",
	        type:"post",
	        data:{
	            id:$("#proposalId").val(),
	            type:type
	        ...
	        success : function(url) {
	            if(url){
	                $("#proposalPdf").attr("href","http://lanpubucket1.oss-cn-beijing.aliyuncs.com/"+url);
	            }
	        }
	    }); 
}
function noticePdf(type){
	   $.ajax({
	        url:"${pageContext.request.contextPath}/supervisionAndCheck/getNoticePdfUrl.do",
	        type:"post",
	        data:{
	            id:$("#noticeId").val(),
	            type:type
	        ...
	        success : function(url) {
	            if(url){
	                $("#noticePdf").attr("href","http://lanpubucket1.oss-cn-beijing.aliyuncs.com/"+url);
	            }
	        }
	    }); 
}
	...
	$.ajax({
		...
		success : function(json) {
			if(json>0){
				...
				$("input[name='inspectionRecordId']").val(json);
...
function findInspectionRecordNum(){
	$.ajax({
		url:"${pageContext.request.contextPath}/supervisionAndCheck/findInspectionRecord.do",
		type:"post",
		dataType:"json",
...
function insertProposal(){
	...
	$.ajax({
		...
		success : function(json) {
			if(json>0){
				alertEase("添加信息成功",'alert_succ');
				$("input[name='proposalId']").val(json);
	...
	if($("#notice").val()){
		...
		$.ajax({
			...
			success : function(json) {
				...
				$("#data_notice").find("#div_illegalContents").html(json.illegalcontents);
			}
		});
	}else{
...
//查询违法条款 
function queryChooseIllegalClause2(curr){
	$("#dataListTbody_chooseIllegalClause2 tr").remove();
    var pageNum = 1;
    if(!curr){
        //初始化分页
        $.ajax({
            ...
            async: false,
            data:{
                illegalClauseCode:$("#searchIllegalClauseCode2").val()
            },
            success : function(total) {
                page("page_illegalClause2",total,10,10,queryChooseIllegalClause2);
            }
        });
    }else{
        pageNum = curr;
    ...
    $.ajax({
        ...
        success : function(json) {
            ...
            $("#dataListTbody_chooseIllegalClause2").append(tr);
        }
    });
}
//选择违法条款 
...
function loadImg(){
	...
	if(imgUrlStr&&imgUrlStr!="null"){
		    ...
		    for(var j=0;j<imgUrls.length;j++) {
		    	if(imgUrls[j]){
		    		  imgA+="<div style='display:inline;'><img style='display:inline;width:140px;height:140px' src='http://lanpubucket1.oss-cn-beijing.aliyuncs.com/"+imgUrls[j]+"'><a href='javascript:void(0)' class='form_btn' onclick='deleteFile(this,\""+imgUrls[j]+"\")'>删除</a></div>";
		    	}
		    }
		    $("#div_img").html(imgA);
	}
...
function upload() {
    ...
    $.ajax({
        ...
        success : function(data) {
            if(data){
                $("input[name='imgUrl']").val($("input[name='imgUrl']").val()+","+data)
            }
        },
        error:function(){
            alertEase("保存失败",2);
